    ...
    <div class="form-wrapper mt-5">
      ...
      <div class="form-container">
        ...
        <form @submit.prevent="submitSection">
          ...
          <button type="submit" class="btn">Submit Section</button>
        </form>
      </div>
      ...
      <div class="form-container">
        ...
        <form @submit.prevent="submitProjectItem">
          ...
          <button type="submit" class="btn" :disabled="!sectionId">Submit Item</button>
        </form>
      </div>
    </div>
  </template>
    ...
    data() {
      return {
        formData: {
          ...
          mainDescription: "",
          project: this.$route.query.projectId || "" // Get project ID from the route query parameter
        },
        projectItem: {
          itemno: "",
          subDescription: "",
          ...
          amount: null,
          wt_percent: null,
          header_per_project_section: null // Will be set after section creation
        },
        sectionId: null // Holds the newly created section ID
      };
    },
    async created() {
    ...
    methods: {
      ...
      async submitProjectItem() {
        try {
          ...
          const itemResponse = await axios.post("http://localhost:1337/api/project-items", {
            data: this.projectItem
          });
  
          console.log("Project item created:", itemResponse.data);
